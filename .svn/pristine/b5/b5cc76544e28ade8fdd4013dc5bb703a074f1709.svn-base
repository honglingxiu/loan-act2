    ...
    <div>
        <ul class="main-form">
            <li class="form-item flex phone-item">
                ...
                <input type="tel" maxlength="11" v-model.trim="formData.phone" placeholder="请输入手机号" class="form-item-input" @input="setCustomPhone(formData.phone)">
            </li>
            <li class="form-item flex">
                ...
                <span class="fs12 c999">万元</span>
            </li>
            <li class="form-item flex">
                <label class="form-item-title">借款时间</label>
                <label v-for="item in loanTimeList" :key="item" class="flex-auto">
                    <input class="loan-time-radio" type="radio" :value="item" v-model="formData.applyDeadline" style="display:none">
                    <span class="radio-btn">{{item+'个月'}}</span>
                </label>
            </li>
            ...
            <div class="select-item form-item flex">
                ...
                <div class="form-item-input" :class="{cccc:formData.cityDowntown===''}" @click="$refs.citypage.open()">{{formData.cityDowntown===''?'请选择城市':formData.cityDowntown}}</div>
            </div>
            ...
            <li class="select-item form-item flex" @click="$refs.picker.open()">
                ...
                <div class="form-item-input" :class="{cccc:formData.birthDate===''}">{{formData.birthDate===''?'请选择您的生日':formData.birthDate}}</div>
                <!-- <input type="text" v-model="formData.birthDate" placeholder="请选择您的生日" readonly class="form-item-input"> -->
            </li>
        </ul>
        ...
        <!-- 城市选择页 -->
        <jb-selectCity v-model="formData.cityDowntown" cityName ref="citypage"></jb-selectCity>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            pickState: "",
            loanTimeList: jsonData.loanTimeList,
            popupSelect: false,
            timer1: "",
            applyId: "",
            formData: {
            ...
            return new Promise(resolve => {
                ...
                this.$api.vkcPost("supermarketloan/distribute/loan/apply", param, "getAllData").then(res => {
                    if (res.code === "0") {
                        ...
                        _paq.push(["trackEvent", idList.channelName + "：" + idList.channelChildName, "预约借款按钮"]); //matomo预约借款按钮统计
    ...
    mounted() {
        this.$nextTick(function() {
            this.resetData();
            if (!this.form3) return;
            let userPhone = window.sessionStorage.getItem("userPhone");
            userPhone ? (this.formData.phone = userPhone) : this.$router.back();
        });
    }
};
</script>
